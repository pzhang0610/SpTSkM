...
class CelebNet(nn.Module):
    def __init__(self, num_classes, num_features=0, dropout=0.6, set_pooling='max'):
        ...
        # self.attenblock0 = CrossAttention(in_channels=num_ftrs, out_channels=num_ftrs)
        self.attenblock0 = SpaceTimeAtten(in_channels=num_ftrs, out_channels=num_ftrs)
        ...
        self.attenblock1 = SpaceTimeAtten(in_channels=num_ftrs//2, out_channels=num_ftrs //2)
        #self.attenblock1 = CrossAttention(in_channels=num_ftrs // 2, out_channels=num_ftrs // 2)
        ...
        # self.attenblock2 = CrossAttention(in_channels=num_ftrs//2, out_channels=num_ftrs//2)
        self.attenblock2 = SpaceTimeAtten(in_channels=num_ftrs//2, out_channels=num_ftrs//2)
        num_ftrs = num_ftrs // 2
        ...
        if self.dropout > 0:
            ...
            self.mask_drop = nn.Dropout(dropout)
            self.glob_drop = nn.Dropout(dropout)
        if self.num_class > 0:
    ...
    def forward(self, vids, masks, out_feature=False):
        ...
        # x : N, C, T, H, W
        x_3d = x.view(n_spl, n_fr, x.shape[1], x.shape[2], x.shape[3]).permute(0, 2, 1, 3, 4)
        m_3d = m.view(n_spl, n_fr, m.shape[1], m.shape[2], m.shape[3]).permute(0, 2, 1, 3, 4)
        g = self.attenblock0(x_3d, m_3d)
        g = torch.max(g.permute(0, 2, 1, 3, 4), 1)[0]

        x = self.conv_layer1(x)
        m = self.mask_layer1(m)
        x_3d = x.view(n_spl, n_fr, x.shape[1], x.shape[2], x.shape[3]).permute(0, 2, 1, 3, 4)
        m_3d = m.view(n_spl, n_fr, m.shape[1], m.shape[2], m.shape[3]).permute(0, 2, 1, 3, 4)
        g = self.glob_layer1(g) + torch.max(self.attenblock1(x_3d, m_3d).permute(0, 2, 1, 3, 4), 1)[0]
        ...
        m_3d = m.view(n_spl, n_fr, m.shape[1], m.shape[2], m.shape[3]).permute(0, 2, 1, 3, 4)
        # pdb.set_trace()
        g = self.glob_layer2(g) + torch.max(self.attenblock2(x_3d, m_3d).permute(0, 2, 1, 3, 4), 1)[0]
...
class SpaceTimeAtten(nn.Module):
    def __init__(self, in_channels, out_channels, bn_layer=True):
        ...
        self.f = nn.Conv3d(in_channels, out_channels, kernel_size=1)
        self.g = nn.Conv3d(in_channels, out_channels, kernel_size=1)
        self.h = nn.Conv3d(in_channels, out_channels, kernel_size=1)
        self.m = nn.Conv3d(in_channels, out_channels, kernel_size=1)
        if bn_layer:
